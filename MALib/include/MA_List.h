...
private:
	...
	LISTNODE<T>* find(uint index);
	LISTNODE<T>* find(T* item, uint* outIndex = 0);
...
template <class T> uint LIST<T>::add(const T& item)
{
...
template <class T> void LIST<T>::insert(const T& item, uint index)
{
...
template <class T> T& LIST<T>::get(uint index)
{
	if (index >= this->size) index = this->size - 1;
	LISTNODE<T>* node = this->find(index);
	return *node->data;
}
template <class T> bool LIST<T>::contains(const T& item, uint* outIndex)
{
...
template <class T> uint LIST<T>::length()
{
	return this->size;
}

template <class T> LISTNODE<T>* LIST<T>::find(uint index)
{
	uint i = 0;
	LISTNODE<T>* current = this->first;
	if (current == 0) 
...
template <class T> LISTNODE<T>* LIST<T>::find(T* item, uint* outIndex)
{
	uint i = 0;
	LISTNODE<T>* current = this->first;
	if (current == 0) 
...
template <class T> T& LIST<T>::operator[](uint index)
{
